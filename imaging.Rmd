---
title: Biostatistics for Imaging Biomarkers and Intro to Longitudinal Data
output: 
...
# Biostatistics for Imaging Biomarkers & Intro to Longitudinal Data
...
* In Alzheimer's Disease (AD) clinical trials we typically assess study participants _longitudinally_ or _repeatedly_ over time
* This gives rise to _serial observations_ for each participant at various time points post baseline
* We cannot treat these serial observations as if they came from different people
* Our analysis methods must account for _within-subject correlation_
* There are specialized statistical methods to help accommodate many varieties of _correlated_ or _clustered_ data
* We will explore _longitudinal data analysis_ approaches commonly employed in AD clinical trials
* We will demonstrate these methods on a _simulated_ clinical trial dataset 
...
```{r echo=FALSE}
fit_adni <- lme(ADAS11 ~ PTGENDER + scale(AGE,scale=FALSE) + I(Years.bl/12), 
...
# Let's simulate a hypothetical clinical trial $\ldots$
...
* A treatment which slows ADAS-Cog progression by 12.5%
* Typical attrition ( $\approx$30%)

---

# Let's simulate a hypothetical clinical trial $\ldots$
Simulation _reverses_ the usual process of statistical modeling/estimation
...
* CAUTION: Simulations can only provide information about _models_, but they cannot 
provide information about _reality_. _Real data_ is required for the latter.
...
Beta <- c(
   '(Intercept)'=20   , # mean ADAS at baseline
        'female'=-0.29, # better scores for females
         ...
         'month'= 0.57, # worse per month post baseline
...
ggplot(trial_obs, aes(x=month, y=ADAS11, group=id, color=group)) + 
  ...
  theme(legend.position=c(0.1, 0.85), legend.background = element_rect())
```
...
# Two sample $t$-test of mean change at month 18 (completers analysis)
...
* `r n2` + `r n1` - 2 = `r DF` "degrees of freedom"
...
$p$-value is area under curve for $|x|>`r abs(tt)`$, the value of the test statistic in this case.
...
* "_Regression_" generally refers to a relationship between variables that is estimated by data
* "_Ordinary Least Squares_" regression, for example, describes a linear relationship between two continuous variables that is estimated by the line that minimizes the sum of squared "residuals"
* _predictor/covariate_ $\rightarrow$ _response/outcome_
* "_Residuals_" are the differences between observations and values predicted by the regression
...
* "_General linear models_" can add multiple _covariates/predictors_
* "_Generalized linear models_" can accommodate other types of _outcome/response_ variables (e.g. logistic regression can accommodate binary outcome variables)
* "_Mixed-effects models_" mix _random effects_ with the standard _fixed effects_ to account for complex correlation structures

All regression models share the common theme of estimating the best fit relationship between
_outcome/response_ variables and _covariates/predictors_
...
* Very common for two groups, and _one_ post- assessment
...
* Ubiquitous, simple, powerful framework
...
```{r}
save(trial, trial_obs, trial_wide, trial_mmrm, attrition_rate, n, 
