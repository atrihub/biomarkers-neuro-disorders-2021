...
# For ADNIMERGE, go to http://adni.loni.usc.edu/
...
```{r, fig.height=4.5, fig.width=4.5*16/9}
par(mfrow=c(1,2))
...
```{r, fig.height=4.5, fig.width=4.5, out.width='100%'}
...
- Minimum Detectable Concentration (MDC) is the lowest concentration where the curve is increasing (decreasing)
...
- Reliable Detection Limit (RDL) for an increasing (decreasing) curve, is the lowest concentration that has a high probability of producing a response that is significantly greater (less) than the response at 0.
...
- Limit of Quantitization (LOQ) is the lowest concentration at which the coefficient of variation is less than a fixed percent (default is 20% in the `calibFit` package).
...
```{r}
logistic.fit <- glm((DX=='Dementia') ~ scale(ABETA) + scale(TAU), data = subset(dd, DX!='MCI'))
summary(logistic.fit)$coef %>% as.data.frame() %>%
...
```{r}
logistic.fit2 <- glm((DX=='Dementia') ~ scale(ABETA) + scale(TAU) + scale(I(AGE+Years.bl)) + as.factor(APOE4), data = subset(dd, DX!='MCI'))
...
```{r, fig.height=5, fig.width=5*(16/9)}
library(party)
