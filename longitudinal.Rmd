...
Instead of:
* Stage 1: $\textrm{ADAS}_{ij} = \beta_{0i} + t_{ij}\beta_{1i} + \varepsilon_{ij}$
* Stage 2: $\hat\beta_{1i} = X_i\beta + \varepsilon'_{ij}$

We simulatenously estimate both levels:
...
* MMRM drops the $b$ terms and can assume _heteroscedastic_ (heterogeneous variance), correlated residuals
...
* _MMRM_ has _change from baseline_ as outcome, baseline assessment as covariate
...
* _cLDA_ uses raw assessment scores as outcome; baseline assessment as outcome, constrains two groups to have same mean
...
```{r echo = FALSE, size = 'scriptsize'}
fits_mi <- with(data=trial_imp, lm(ADAS11.m18~active*scale(ADAS11.m0, scale = FALSE)))
summary(fits_mi)
```
...
for (k in 1:length(k_tipping)){
  ...
  fit_k <- with(imp_k, lm(ADAS11.m18~active*scale(ADAS11.m0, scale=FALSE)))
...
* _Estimand_: Target of estimation 
    e.g. the treatment effect in ITT population
* _Estimator_: Rules for estimation
    e.g. final visit contrast based on MMRM
* _Estimate_: The number derived from applying estimator to data
