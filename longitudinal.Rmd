...
# Crossed (non-nested) random effects (e.g. FreeSurfer version)

```{r}
hippvol_fs <- hippvol %>% 
  filter(!is.na(FSVERSION)) %>%
  filter(!DX.bl %in% c('SMC', 'EMCI')) %>%
  mutate(DX.bl = factor(DX.bl, levels = c('CN', 'LMCI', 'AD')))

n_crossed <- length(intersect(
  filter(hippvol_fs, FSVERSION == 'Cross-Sectional FreeSurfer (FreeSurfer Version 4.3)')$RID,
  filter(hippvol_fs, FSVERSION == 'Cross-Sectional FreeSurfer (5.1)')$RID
))

ggplot(hippvol_fs, aes(x=yrs, y=Hippocampus, color=FSVERSION, group=RID)) +
  geom_line(alpha=0.5) +
  facet_wrap(.~DX.bl, scales = 'free_x')
```

We see `r n_crossed` subjects with data from both FreeSurfer versions

---

# Testing for crossed random effects

```{r}
library(lme4)
hipp_fit_lmer <- lmer(Hippocampus ~ yrs*DX.bl + AGE + PTGENDER + scale(ICV.bl) +
    (yrs | RID), data=hippvol_fs)

hipp_fit_lmer_fs <- lmer(Hippocampus ~ yrs*DX.bl + AGE + PTGENDER + scale(ICV.bl) + 
    FSVERSION + (1 | FSVERSION) + (yrs | RID), data=hippvol_fs)

anova(hipp_fit_lmer, hipp_fit_lmer_fs)
```
...
```{r}
summary(hipp_fit_lmer_fs)
```

---

# Crossed and nested random effects (Site & FreeSurfer)

```{r}
hipp_fit_lmer_fs_site <- lmer(Hippocampus ~ yrs*DX.bl + AGE + PTGENDER + scale(ICV.bl) + 
    (1 | FSVERSION) + (yrs | SITE/RID), data=hippvol_fs)

anova(hipp_fit_lmer_fs, hipp_fit_lmer_fs_site)
```
